...
#             logging.info('read_col_cnt[{}]: {}'.format(logic_col, read_col_cnt[logic_col]))
            if read_col_cnt[logic_col] >= READ_COL-1:
                max_bx_latency = np.amax(pix_mem_bx[col_start:col_end][np.where(pix_mem_tot[col_start:col_end] == 0)])
...
#                          pix_mem_bx[to_read_pix[0][read_pix]][to_read_pix[1][read_pix]] <= (PIXEL_MAX_LATENCY + pix_mem_tot[to_read_pix[0][read_pix]][to_read_pix[1][read_pix]]):
...
#                             logging.info('col ro delay: {}'.format(pix_mem_bx[to_read_pix[0][read_pix]][to_read_pix[1][read_pix]]))
                    ...
                    break
            elif np.amax(pix_mem_bx[col_start:col_end][np.where(pix_mem_tot[col_start:col_end] == 0)]) > 0:
